...
const TUPPER_REPLY_REGEX = new RegExp(
  /^\> \[Reply to\]\(<https:\/\/discord.com\/channels\/.+>\):.+$\n^> .+/m
);
...
async function parseMessages(channel, startPost = null, endPost = null, capNumber = null) {
    ...
    while (pointer) {
      await channel.messages.fetch({ limit: 100, before: pointer.id }).then((messagePage) => {
        ...
        if (capNumber.toLowerCase() !== "all" && messages.size >= parseInt(capNumber)) {
          ...
          startPost &&
          startPost.createdTimestamp > messagePage.at(messagePage.size - 1)?.createdTimestamp
        ) {
          pointer = null;
        } else if (
          endPost &&
          endPost.createdTimestamp < messagePage.at(messagePage.size - 1)?.createdTimestamp
        ) {
          pointer = null;
  ...
  messages.forEach((message) => {
    ...
    if (message.content) {
      const reply = message.content.replace(TUPPER_REPLY_REGEX, "").trim();
      wc += reply.split(" ").filter((i) => i !== "").length;
    }
...
function generateReport(channel, participants, textlog, messages, startPost, skipBot = false) {
  ...
  const description =
    (messages.size === 100
      ? "!!!This is a log snippet capped at 100 messages, please enable `increasecap` if you want to see more messages.\n\n"
      : "") +
    "**Channel:** <#" +
    channel.id +
    ">" +
  ...
  content += `\nLog URL: https://discord.com/channels/${channel.guild.id}/${channel.id}${
    startPost > 0 ? "/" + startPost : ""
  }`;
  content += `\nTotal (non-empty) Messages: ${textlog.length} (out of ${messages.size} parsed)`;
  content += `\nTotal Word Count: ${Object.values(participants).reduce(
    (sum, a) => sum + a.wc,
    0
  )}\nTotal Participants: ${Object.keys(participants).length}`;
  content += `\nParticipants:\n  - ${Object.values(participants)
    .sort((a, b) => b.wc - a.wc)
    .map((i) => `${i.displayname} (WC: ${i.wc}, Files: ${i.totalpics})`)
    .join("\n  - ")}`;
  content += "\n";
...
function exportAsMarkdown(channel, participants, textlog) {
  let content =
    `# ${channel.name}\n\n` +
    `- Summary\n\n` +
    `    [Start](https://discord.com/channels/${channel.guild.id}/${channel.id}/${
      textlog[0].id
    }) → [End](https://discord.com/channels/${channel.guild.id}/${channel.id}/${
      textlog[textlog.length - 1].id
    })\n\n` +
    `    **Date:** ${new Date(textlog[0].timestamp).toLocaleString("en-US", {
      timeZone: "CST",
    })} → ${new Date(textlog[textlog.length - 1].timestamp).toLocaleString("en-US", {
      timeZone: "CST",
    })}\n\n` +
